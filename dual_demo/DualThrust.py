...
class Dual_Thrust(object):
    ...
    def trading_signal(self):
        ...
        self.data['signal']=2
        self.data['Long_signal'] = np.where(self.data['high'] > self.data['BuyLine'], 1, 2)
        self.data['Short_signal'] = np.where(self.data['low'] < self.data['SellLine'], 0, 3)
        df=self.data['Long_signal']+self.data['Short_signal']
        df[df<=2]=0
        df[df ==4 ]= 1
        df[df >4 ]= 2
    ...
    def buy(self,row):
        ...
        print('Buying, account status after trading is: Cash:%s,  holding:%s' % (self.cash, self.holding))
        
    def sell(self,row):
        # 发出卖出动作，按照当前low价格结算为对应
        ...
        self.data.iloc[row,self.data.columns.get_loc('Trading_action')] = 1
        print('Trading Date is: %s' % self.data.iloc[row,self.data.columns.get_loc('Date')])
        print('Selling, account status after trading is: Cash:%s, holding:%s' % (self.cash, self.holding))
        
    def update_cap(self,row):
        #计算当前资本
        self.data.iloc[row,self.data.columns.get_loc('cash')] = self.cash
        self.data.iloc[row,self.data.columns.get_loc('quantity')] = self.holding
    ...
    def show_return(self):
        ...
        fig.suptitle('Arbitrage Performance Curve', fontsize=20)
    ...
    def trading(self):
        ...
        for i in range(0,len(self.data)):
            if self.data.iloc[i,self.data.columns.get_loc('signal')] == 0:
                if self.holding:
                    self.sell(i)
            elif self.data.iloc[i,self.data.columns.get_loc('signal')] == 1:
                if not self.holding:
                    self.buy(i)
            else:
                pass
            self.update_cap(i)
        self.data[['Date','signal']].to_csv('data.csv')
